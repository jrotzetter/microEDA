...
#' @param sample_names_col Column name in `metadata` to use as sample identifiers.
#' @importFrom utils tail
#'
#' @return An object of class `microEDA`.
#' @keywords internal
#' @noRd
.metaphlanConstructor <- function(tax_profile, metadata = NULL, sample_names_col = NULL) {
  # TODO: add check to make sure there are no negative values
  ...
  } else if (.mpa_check_names(tax_profile@mpa_taxProfile, metadata, sample_names_col)) {
    rownames(metadata) <- NULL
    metadata <- tibble::column_to_rownames(metadata, sample_names_col)
...
#'
#' Internal function to create a microEDA object by combining a otu_table
...
#' @keywords internal
#' @noRd
.phyloseqConstructor <- function(tax_profile, metadata = NULL, sample_names_col = NULL) {
  # TODO: add check to make sure there are no negative values
  ...
  if (!is.null(sample_names_col) & is.null(metadata)) .show_error('"metadata" must not be NULL when "sample_names_col" is specified.')

  if (is.null(metadata)) {
    metadata <- phyloseq::sample_data(tax_profile)
  }
...
#' @param sample_names_col Column name to use as sample identifiers.
#' @return An object of class `microEDA`.
...
setMethod("microEDA", "metaphlanProfile", function(tax_profile,
  ...
  .metaphlanConstructor(tax_profile,
    metadata = metadata,
    sample_names_col = sample_names_col
  )
})
